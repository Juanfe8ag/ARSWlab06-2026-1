...
export default function BlueprintsPage() {
  ...
  useEffect(() => {
    if (current?.points) {
      setOriginalPoints(current.points)
      setEditedPoints(current.points)
    }
  }, [current])
  ...
  const handleDeleteBlueprint = (bp) => {
  ...
  const savePoints = async () => {
      ...
      const newPoints = editedPoints.slice(originalPoints.length)
    ...
    return (
      <div className="grid" style={{ gridTemplateColumns: '1.1fr 1.4fr', gap: 24 }}>
          ...
          <div className="card">
            ...
            {status === 'failed' && (
              <div style={{ marginTop: 10 }}>
                <p style={{ color: 'red' }}>Error cargando blueprints</p>
                <p>{error}</p>
                <button
                  className="btn"
            ...
            {!items.length && status === 'succeeded' && <p>Sin resultados.</p>}
            {!!items.length && (
                  ...
                  {items.map((bp) => (
                    <tr key={bp.name}>
                      ...
                      <td style={{ padding: '8px', borderBottom: '1px solid #1f2937' }}>
                        ...
                        <button className="btn" onClick={() => handleDeleteBlueprint(bp)}>
                          Delete
                        </button>
                      </td>
                    </tr>
        ...
        <section className="card">
          ...
          <BlueprintCanvas points={editedPoints} onChange={setEditedPoints} />
          <button
